...
lemma middle_steps' (hm3 : 3 ≤ m) (v : Vertex m)
    ...
    v'.2.1 = (if v.1 = -1 then v.2.1 else v.2.1 - 2) := by
      ...
      have h_ind : ∀ k ≤ m - 2, ((step m 0)^[k] v).1 = v.1 ∧ ((step m 0)^[k] v).2.1 = if v.1 = -1 then v.2.1 else v.2.1 + k := by
        intro k hk_le; induction' k with k ih <;> simp_all +decide [ Function.iterate_succ_apply' ] ;
        specialize ih ( Nat.le_of_succ_le hk_le ) ; specialize h_fiber_k k ( Nat.le_of_succ_le hk_le ) ; simp_all +decide [ ← add_assoc ] ;
        have h_step : fiber m ((step m 0)^[k] v) ≠ 0 ∧ fiber m ((step m 0)^[k] v) ≠ -1 := by
          constructor <;> intro h <;> simp_all +decide [ add_eq_zero_iff_eq_neg ];
          · norm_cast at h_fiber_k;
            rw [ eq_comm, ZMod.natCast_eq_zero_iff ] at h_fiber_k ; linarith [ Nat.le_of_dvd ( by linarith ) h_fiber_k, Nat.sub_add_cancel ( by linarith : 2 ≤ m ) ];
          · rw [ eq_comm ] at h_fiber_k;
            rw [ eq_neg_iff_add_eq_zero ] at h_fiber_k;
            norm_cast at h_fiber_k;
            rw [ ZMod.natCast_eq_zero_iff ] at h_fiber_k ; linarith [ Nat.le_of_dvd ( by linarith ) h_fiber_k, Nat.sub_add_cancel ( by linarith : 2 ≤ m ) ];
...
lemma return_map_eq_model (hm3 : 3 ≤ m) (v : Vertex m) (h : fiber m v = 0) :
    ...
    (v'.1, v'.2.1) = next_state m (v.1, v.2.1) := by
      ...
      obtain ⟨v_end, hv_end⟩ : ∃ v_end : Vertex m, (step m 0)^[m - 2] v1 = v_end ∧ fiber m v_end = -1 ∧ v_end.1 = v1.1 ∧ v_end.2.1 = (if v1.1 = -1 then v1.2.1 else v1.2.1 - 2) := by
        have h_fiber_iter := iterate_fiber m 0 v1
        have := h_fiber_iter ( m - 2 ) ; rcases m with ( _ | _ | m ) <;> simp_all +decide ;
        exact ⟨ by rw [ eq_neg_iff_add_eq_zero ] ; norm_cast; simp +arith +decide, by simpa [ hv1.2 ] using middle_steps' ( m + 2 ) ( Nat.le_trans ( by decide ) ( Nat.add_le_add_right hm3 2 ) ) v1 hv1.2 ⟩;
      obtain ⟨v', hv'⟩ : ∃ v' : Vertex m, step m 0 v_end = v' ∧ (v'.1, v'.2.1) = next_state m (v.1, v.2.1) := by
...
lemma next_state_fst (v : ZMod m × ZMod m) (h : v.2 = -1) :
    (next_state m v).1 = v.1 + 1 := by
      have h_next_state : next_state m v = if v.1 = -2 then (-1, 0) else if v.1 = -1 then (0, -3) else (v.1 + 1, -2) := by
...
lemma column_y_avoid_neg_one (hm : Odd m) (x : ZMod m) (k : ℕ) (hk : k < m - 1) :
    entry_point m x + k * column_delta m x ≠ -1 := by
      ...
      · rw [ eq_neg_iff_add_eq_zero ];
        norm_cast;
        rw [ ZMod.natCast_eq_zero_iff ] ; exact Nat.not_dvd_of_pos_of_lt ( Nat.succ_pos _ ) ( Nat.lt_pred_iff.mp hk );
      · intro h
        ...
        have h_eq' : (k : ZMod m) = -1 := by
          ...
          have h_inv : ∃ y : ZMod m, y * 2 = 1 := by
            have h_inv : ∃ y : ℕ, y * 2 ≡ 1 [MOD m] := by
              exact ⟨ ( m + 1 ) / 2, by rw [ Nat.div_mul_cancel ( even_iff_two_dvd.mp ( by simpa [ parity_simps ] using hm ) ) ] ; norm_num [ Nat.ModEq ] ⟩;
            exact ⟨ h_inv.choose, by simpa [ ← ZMod.natCast_eq_natCast_iff ] using h_inv.choose_spec ⟩;
          exact eq_of_sub_eq_zero ( by obtain ⟨ y, hy ⟩ := h_inv; linear_combination' h_eq' * y - hy * ( k - -1 ) )
      ...
      · intro h; rw [ neg_add_eq_sub ] at h; rw [ sub_eq_iff_eq_add ] at h; norm_num at h;
        ...
        norm_cast at h;
        rw [ ZMod.natCast_eq_zero_iff ] at h ; exact Nat.not_dvd_of_pos_of_lt ( Nat.succ_pos _ ) ( Nat.lt_pred_iff.mp hk ) h
...
lemma column_traversal (hm : Odd m) (x : ZMod m) :
    ...
    ((next_state m)^[m] (x, start)).1 = x + 1 := by
      have h_avoid : ∀ k < m - 1, let start := entry_point m x; let delta := column_delta m x; start + k * delta ≠ -1 := by
        exact fun k hk => column_y_avoid_neg_one m hm x k hk
      have h_induction : ∀ k < m, let start := entry_point m x; let delta := column_delta m x; (next_state m)^[k] (x, start) = (x, start + k * delta) := by
        intro k hk
        induction' k with k ih
        all_goals generalize_proofs at *;
        · norm_num;
        · specialize ih ( Nat.lt_of_succ_lt hk ) ; simp_all +decide [ Function.iterate_succ_apply' ] ;
          unfold next_state; simp +decide [ add_mul, add_assoc ] ;
          split_ifs <;> simp_all +decide [ ← add_assoc ];
          all_goals have := h_avoid k ( Nat.lt_pred_iff.mpr hk ) ; simp_all +decide [ column_delta ];
          · ring;
          · ring;
      specialize h_induction ( m - 1 ) ; rcases m with ( _ | _ | m ) <;> simp_all +decide [ Function.iterate_succ_apply' ] ;
      · fin_cases x ; trivial;
      · refine' ⟨ fun k hk => _, _ ⟩;
        · induction' k with k ih;
          · norm_num;
        ...
        · convert next_state_fst (m + 1 + 1) _ _ using 1;
          ...
          split_ifs <;> norm_num [ show ( m : ZMod ( m + 1 + 1 ) ) = -2 by { exact eq_neg_of_add_eq_zero_left ( by norm_cast; simp +arith +decide ) } ]

set_option maxHeartbeats 800000 in
lemma column_step (hm : Odd m) (x : ZMod m) :
    (next_state m)^[m] (x, entry_point m x) = (x + 1, entry_point m (x + 1)) := by
      have h_col_traversal : ∀ k < m, (next_state m)^[k] (x, entry_point m x) = (x, entry_point m x + k * column_delta m x) := by
      ...
      have h_col_traversal_last : (next_state m)^[m - 1] (x, entry_point m x) = (x, -1) := by
        ...
        unfold column_delta entry_point; norm_num [ hm, Nat.even_sub ( show 1 ≤ m from NeZero.pos m ) ] ;
        split_ifs <;> norm_num [ Nat.cast_sub ( show 1 ≤ m from NeZero.pos m ) ];
...
lemma delta_is_unit (hm : Odd m) (x : ZMod m) :
    IsUnit (column_delta m x) := by
      obtain ⟨u, hu⟩ : ∃ u : ZMod m, 2 * u = 1 := by
        ...
        exact ⟨ h_inv.choose, by simpa [ ← ZMod.intCast_eq_intCast_iff ] using h_inv.choose_spec ⟩;
      unfold column_delta;
...
lemma orbit_formula (hm : Odd m) (i : ℕ) (j : ℕ) (hi : i < m) (hj : j < m) :
    ...
    ((i : ZMod m), entry_point m i + (j : ZMod m) * column_delta m i) := by
      ...
      · convert (column_traversal m hm ( i + 1 )).1 j hj using 1;
        · convert congr_arg ( fun x : ZMod m × ZMod m => ( next_state m )^[j] x ) ( column_step m hm i ) using 1;
          rw [ ← Function.iterate_add_apply ] ; ring;
          convert congr_arg ( fun x : ZMod m × ZMod m => ( next_state m )^[m + j] x ) ( ih 0 ( Nat.lt_of_succ_lt hi ) ( Nat.zero_lt_of_lt hj ) ) using 1 ; ring;
...
lemma model_hamiltonian (hm : Odd m) (hm3 : 3 ≤ m) :
    ∀ x : ZMod m × ZMod m, Function.minimalPeriod (next_state m) x = m ^ 2 := by
      have h_start : Function.minimalPeriod (next_state m) (0, entry_point m 0) = m^2 := by
        ...
        · simp_all +decide [ Nat.find_eq_iff ];
          ...
          · rw [ sq, Function.IsPeriodicPt, Function.IsFixedPt ];
            have h_step : ∀ x : ZMod m, (next_state m)^[m] (x, entry_point m x) = (x + 1, entry_point m (x + 1)) := by
              exact column_step m hm
            have h_step_m : ∀ x : ZMod m, ∀ k : ℕ, (next_state m)^[k * m] (x, entry_point m x) = (x + k, entry_point m (x + k)) := by
              intro x k; induction k <;> simp_all +decide [ Nat.succ_mul, ← add_assoc ] ;
              rw [ add_comm, Function.iterate_add_apply, ‹ ( next_state m ) ^[ _ * m ] ( x, entry_point m x ) = _ ›, h_step ];
          ...
          · intro n hn hn_pos hn_periodic
            ...
            obtain ⟨i, j, hi, hj, hn_eq⟩ : ∃ i j : ℕ, i < m ∧ j < m ∧ n = i * m + j := by
              exact ⟨ n / m, n % m, Nat.div_lt_of_lt_mul <| by linarith, Nat.mod_lt _ <| by positivity, by rw [ Nat.div_add_mod' ] ⟩;
            have h_orbit_eq : (i : ZMod m) = 0 ∧ entry_point m i + j * column_delta m i = entry_point m 0 := by
              have h_orbit_eq : (next_state m)^[i * m + j] (0, entry_point m 0) = ((i : ZMod m), entry_point m i + (j : ZMod m) * column_delta m i) := by
                convert orbit_formula m hm i j hi hj using 1;
              aesop;
            rw [ ZMod.natCast_eq_zero_iff ] at h_orbit_eq ; simp_all +decide [ Nat.dvd_iff_mod_eq_zero, Nat.mod_eq_of_lt ];
            simp_all +decide [ entry_point, column_delta ];
            have h_contra : (j : ZMod m) = 0 := by
              have h_contra : IsUnit (2 : ZMod m) := by
                have h_contra : ∃ k : ℕ, 2 * k ≡ 1 [MOD m] := by
                  exact ⟨ ( m + 1 ) / 2, by rw [ mul_comm, Nat.div_mul_cancel ( even_iff_two_dvd.mp ( by simpa [ parity_simps ] using hm ) ) ] ; norm_num [ Nat.ModEq ] ⟩;
                obtain ⟨ k, hk ⟩ := h_contra; exact isUnit_iff_exists_inv.mpr ⟨ k, by simpa [ ← ZMod.natCast_eq_natCast_iff ] using hk ⟩ ;
              exact h_contra.mul_left_inj.mp ( by aesop );
            exact absurd h_contra ( by rw [ ZMod.natCast_eq_zero_iff ] ; exact Nat.not_dvd_of_pos_of_lt hn_pos hj );
        · rename_i h;
          contrapose! h;
          use m^2;
          ...
          have h_step : ∀ x : ZMod m, (next_state m)^[m] (x, entry_point m x) = (x + 1, entry_point m (x + 1)) := by
              exact column_step m hm
          have h_step_m2 : ∀ x : ZMod m, (next_state m)^[m * m] (x, entry_point m x) = (x + m, entry_point m (x + m)) := by
            intro x
            have h_step_m2 : ∀ k : ℕ, (next_state m)^[k * m] (x, entry_point m x) = (x + k, entry_point m (x + k)) := by
              intro k
              induction' k with k ih;
              · norm_num;
              · rw [ Nat.succ_mul, add_comm, Function.iterate_add_apply, ih, h_step ] ; push_cast ; ring;
            exact h_step_m2 m;
          simpa using h_step_m2 0;
      have h_orbit : ∀ x : ZMod m × ZMod m, x ∈ {(next_state m)^[i] (0, entry_point m 0) | i < m^2} := by
        intro x
        obtain ⟨i, j, hi, hj, hx⟩ : ∃ i j : ℕ, i < m ∧ j < m ∧ x = ((i : ZMod m), entry_point m i + (j : ZMod m) * column_delta m i) := by
          obtain ⟨i, j, hi, hj, hx⟩ : ∃ i j : ZMod m, x = (i, entry_point m i + j * column_delta m i) := by
            use x.1;
            have h_unit : IsUnit (column_delta m x.1) := by
              exact delta_is_unit m hm x.1
            obtain ⟨ j, hj ⟩ := h_unit.exists_right_inv;
            exact ⟨ ( x.2 - entry_point m x.1 ) * j, Prod.ext rfl <| by linear_combination' hj.symm * ( x.2 - entry_point m x.1 ) ⟩;
          use i.val, j.val;
      ...
      have h_period : Function.minimalPeriod (next_state m) x = Function.minimalPeriod (next_state m) (0, entry_point m 0) := by
...
lemma period_scaling {S : Type*} (f : S → S) (fib : S → ZMod m)
    ...
    Function.minimalPeriod f x = m * Function.minimalPeriod (f^[m]) x := by
      have h_one_cycle : Function.minimalPeriod f x ∣ m * Function.minimalPeriod (f^[m]) x ∧ m * Function.minimalPeriod (f^[m]) x ∣ Function.minimalPeriod f x := by
        constructor;
        ...
        · have h_min_period : f^[Function.minimalPeriod f x] x = x := by
          ...
          have h_div : m ∣ Function.minimalPeriod f x := by
            have h_fiber_period : fib (f^[Function.minimalPeriod f x] x) = fib x + Function.minimalPeriod f x := by
              refine' Nat.recOn ( Function.minimalPeriod f x ) _ _ <;> simp_all +decide [ Function.iterate_succ_apply', add_assoc ];
            simp_all +decide [ ← ZMod.natCast_eq_zero_iff ];
          obtain ⟨ k, hk ⟩ := h_div; simp_all +decide [ Function.iterate_mul, Function.iterate_fixed ] ;
          exact Nat.mul_dvd_mul_left m ( Function.IsPeriodicPt.minimalPeriod_dvd ( by aesop ) );
...
lemma conjugacy (hm3 : 3 ≤ m) (x : ZMod m × ZMod m) :
    (step m 0)^[m] (model_to_vertex m x) = model_to_vertex m (next_state m x) := by
      ...
      · convert congr_arg Prod.snd ( return_map_eq_model m hm3 ( model_to_vertex m x ) (model_to_vertex_fiber m x) ) using 1;
...
lemma period_conjugacy (hm3 : 3 ≤ m) (x : ZMod m × ZMod m) :
    ...
    Function.minimalPeriod (next_state m) x := by
      ...
      · ext n; simp [Function.IsPeriodicPt];
        intro hn_pos
        have h_conj : ∀ n : ℕ, (step m 0)^[m * n] (model_to_vertex m x) = model_to_vertex m ((next_state m)^[n] x) := by
          intro n
          induction' n with n ih;
          · rfl;
          · rw [ Nat.mul_succ, add_comm, Function.iterate_add_apply, ih, Function.iterate_succ_apply' ];
      ...
      · constructor;
        · rintro ⟨ n, hn, hn' ⟩;
          use n;
          have h_conj : ∀ k : ℕ, (step m 0)^[m * k] (model_to_vertex m x) = model_to_vertex m ((next_state m)^[k] x) := by
            intro k; induction k <;> simp_all +decide [ Nat.mul_succ, Function.iterate_succ_apply' ] ;
            rw [ Nat.add_comm, Function.iterate_add_apply, ‹ ( step m 0 ) ^[ m * _ ] ( model_to_vertex m x ) = model_to_vertex m ( ( next_state m ) ^[ _ ] x ) › ];
        ...
        · rintro ⟨ k, hk ⟩;
          ...
          have h_conj : (step m 0)^[m]^[k] (model_to_vertex m x) = model_to_vertex m ((next_state m)^[k] x) := by
...
theorem cycle0_hamiltonian (hm : Odd m) (hm3 : 3 ≤ m) :
  ...
  have h_period_eq : Function.minimalPeriod f v = Function.minimalPeriod f (f^[k] v) := by
    have h_period_eq : ∀ x : Vertex m, Function.minimalPeriod f x = Function.minimalPeriod f (f x) := by
      intros x
      ...
      exact Nat.dvd_antisymm
        (Function.IsPeriodicPt.minimalPeriod_dvd ((h_period_iff _).mpr (Function.isPeriodicPt_minimalPeriod f (f x))))
        (Function.IsPeriodicPt.minimalPeriod_dvd ((h_period_iff _).mp (Function.isPeriodicPt_minimalPeriod f x)))
